...
class DoubaoAiService extends OpenAiCompatibleAiService {
  DoubaoAiService({
  ...
  @override
  String get defaultModel => 'doubao-1.5-pro-32k';

  @override
  String get defaultVisionModel => 'doubao-1.5-vision-pro-32k';

  @override
  String get defaultAudioModel => 'doubao-asr-2.0';

  /// 豆包音频转录功能
  /// 使用配置的语音模型，调用豆包ASR专用接口
  ...
  Future<Map<String, dynamic>> audioTranscription({
    ...
    try {
      ...
      // 调用豆包ASR接口
      final response = await _callDoubaoAsrApi(
        asrEndpoint,
        audioBytes,
        fileName,
        mimeType,
      ...
      if (e is DioException) {
        ...
        throw Exception(
          '豆包语音识别请求失败 [HTTP $statusCode]: ${responseData?.toString() ?? e.message}',
        );
      }
      rethrow;
    }
  ...
  /// 调用豆包ASR接口（multipart/form-data格式）
  Future<Map<String, dynamic>> _callDoubaoAsrApi(
    String endpoint,
    Uint8List audioBytes,
    String fileName,
    String mimeType,
    ...
    if (resp.data?['error'] != null) {
      ...
      throw Exception('豆包ASR API错误：$message');
    }
